...
#    shapeAlt = ["Pow","Pow","Pow","Pow","Pow"]
    ...
    mass  = [1000,1500,2000]
    ccmlo = [ 900,1400,1900]  
    ccmhi = [1100,1600,2100]  
    mjlo  = [  40,  40,  40]  
    mjhi  = [ 130, 130, 130]  
    mlo   = [ 800, 800, 800]      
    mhi   = [2500,2500,2500]          
    shape    = ["Exp","Exp","Exp"]
    shapeAlt = ["Pow","Pow","Pow"]
    ...
    if options.makeCards:
        ...
        for i in range(mLo,mHi):
            for j in range(cpLo,cpHi):
                ...
                command = "nohup python exo_doFit_class.py %s rsg%03d_kMpl01_py %02d %02d %02d %02d %02d %02d %s %s -b -m --cprime %02d --BRnew 00  &"%(CHAN, mass[i], ccmlo[i], ccmhi[i], mjlo[i], mjhi[i], mlo[i], mhi[i], shape[i], shapeAlt[i], cprime[j]);
