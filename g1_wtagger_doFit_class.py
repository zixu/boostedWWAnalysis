...
class doFit_wj_and_wlvj:
    ...
    def make_Pdf(self, label, in_model_name, mass_spectrum="_mj", ConstraintsList=[], ismc = 0):
        ...
        if in_model_name == "ExpTail":
            ...
            rrv_s_ExpTail = RooRealVar("rrv_s_ExpTail"+label+"_"+self.channel,"rrv_s_ExpTail"+label+"_"+self.channel, 170,10,300);
...
#            xframe_data.addObject(tl_data_sigma);
        ...
        xframe_data_extremefail.GetYaxis().SetRangeUser(1e-2,xframe_data_extremefail.GetMaximum()*1.2);
...
#            xframe_data.addObject(tl_data_sigma);
    ...
    def legend4Plot(self, plot, left=1, isFill=1, xoffset=0., yoffset=0., x_right_offset=0., y_upper_offset=0.):
        ...
        for obj in range(int(plot.numItems()) ):
            ...
            print objName;
            if not (((plot.getInvisible(objName)) and (not TString(objName).Contains("Uncertainty")) ) or TString(objName).Contains("invisi") or TString(objName).Contains("TLine") 
or objName==objName_before ):
    ...
    def fit_TTbar_controlsample(self, isherwig=0):
     ...
     if isherwig ==0:
        ...
        self.fit_mj_single_MC(self.file_WJets0_mc,"_WJets0_failtau2tau1cut","ExpGaus","_TTbar_controlsample"); ## Exp fail sample 
